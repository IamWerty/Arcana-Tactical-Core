...
class ArcanaTacticalCore(ctk.CTk):
    ...
    def run_core_formulas(self):
        try:
            ...
            # Нелінійна маса снаряду.
            #
            # Базова крива для d6, апроксимована по точках таблиці Дамаг Магією:
            #   1d6→4, 2d6→10, 3d6→24, 4d6→40, 6d6→50, 10d6→65, 20d6→100
            #
            # Дві фази:
            #   До критичної маси (N ≤ 4): зростання прискорюється — m = 3.0244 * N^1.8646
            #   Після критичної маси (N > 4): магія живить себе, крива різко сповільнюється —
            #                                  m = 40 + 98.13 * log2((N-4)*0.0329 + 1)
            #
            # Для інших типів кубиків крива масштабується множником ціни куба відносно d6:
            #   d4×0.75, d6×1.0, d8×1.25, d10×1.625, d12×2.0
            def get_mass_d6(n):
                if n <= 0:
                    return 0.0
                elif n <= 4:
            ...
            m = round(get_mass_d6(N_dice) * dk_price_multiplier[dK], 2)
